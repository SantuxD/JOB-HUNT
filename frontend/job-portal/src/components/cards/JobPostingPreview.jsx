...
const JobPostingPreview = ({ formData, setIsPreview }) => {
  ...
  const isRemote = formData.isRemote || formData.jobType === "Remote";
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-blue-50/30 to-purple-50/20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8 backdrop-blur-sm bg-white/80 border border-white/20 shadow-xl roulded-2xl px-6 pt-6">
          ...
          <div className="">
            <div className="relative bg-white px-0 pb-8 mt-8 border-b border-gray-100">
              <div className="relative z-10">
                    ...
                    <div className="flex items-center space-x-4 text-gray-600">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm font-medium ">
                          {isRemote ? "Remote" : formData.location}
                        </span>
                ...
                <div className="flex flex-wrap gap-3 mt-6 md:mt-0 ">
                  ...
                  <span className="px-4 py-2 text-sm bg-purple-50 text-purple-700 font-semibold borde border-purple-700 rounded-full">
            ...
            <div className="px-0 pb-8 space-y-8">
              <div className="relative overflow-hidden bg-linear-to-r from-emerald-50/50 to-teal-50/30 border border-emerald-100/60 p-6 rounded-2xl">
                ...
                <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div className="flex items-center space-x-4">
                    <div className="p-3 bg-linear-to-r from-emerald-500 to-teal-500 rounded-2xl shadow-lg shadow-emerald-100">
                      <DollarSign className="h-6 w-6 text-white" />
                    </div>
                    <div>
                      <h3 className="text-xs font-semibold text-emerald-800 uppercase tracking-wider mb-0.5">
                        Compensation
                      </h3>
                      <div className="text-lg sm:text-xl font-bold text-gray-900 flex items-center">
                        {currencies.find((c) => c.value === formData.currency)
                          ?.label || "$"}
                        {formData.salaryMin && !isNaN(formData.salaryMin)
                          ? Number(formData.salaryMin).toLocaleString()
                          : "0"}{" "}
                        -{" "}
                        {currencies.find((c) => c.value === formData.currency)
                          ?.label || "$"}
                        {formData.salaryMax && !isNaN(formData.salaryMax)
                  ...
                  <div className="flex items-center space-x-2 text-xs sm:text-sm font-semibold text-emerald-700 bg-emerald-100/60 border border-emerald-200/50 px-4.5 py-1.5 rounded-xl self-start sm:self-auto shadow-xs">
